...
-(void)startRequestingFrames {
	if (self.frameDelegate != nil) {
        if ([self.frameDelegate respondsToSelector:@selector(nextFrameInCVPixelBuffer:)]) {
			mode = kMSMovieEncoderCVPixelBufferMode;
		}
		else if ([self.frameDelegate respondsToSelector:@selector(nextFrameInBGRCGBitmapContext:)]) {
	...
	[assetWriterInput requestMediaDataWhenReadyOnQueue:queue usingBlock:^{
        while ([assetWriterInput isReadyForMoreMediaData])
        {
            ...
            if (nextPixelBuffer)
            {
                ...
                if (self.currentTime.value != lastTime) {
                    ...
                    if (![pixelBufferAdaptor appendPixelBuffer:nextPixelBuffer withPresentationTime:currentTime]) {
                        ...
                        [self performSelectorOnMainThread:@selector(encodeAndWriteToDisk) withObject:nil waitUntilDone:YES];
                    }
                    else {
                    ...
                    lastTime = self.currentTime.value;
                }
            ...
            else
            {
				...
				[self performSelectorOnMainThread:@selector(encodeAndWriteToDisk) withObject:nil waitUntilDone:YES];
                //this is blocking because it has the blocking finishWriting call in it. (doesn't matter)
                break;
            }
        }
    }];
...
//        [assetWriter finishWritingWithCompletionHandler:^{
//
//            [self performSelectorOnMainThread:@selector(finishedWriting) withObject:nil waitUntilDone:YES];
//        }];
//        return;
//    }
...
-(CVPixelBufferRef)requestFrameFromDelegate {
	...
	switch (mode) {
		case kMSMovieEncoderBGRCGBitmapContextModeMode: //make a bitmap context and get the delegate to draw in it
            ...
            NSLog(@"Frame Size : %@",NSStringFromCGSize(frameSize));
            if (pxdata) {
                NSLog(@"data : %p",pxdata);
            }
            NSLog(@"color space : %@",rgbColorSpace);
